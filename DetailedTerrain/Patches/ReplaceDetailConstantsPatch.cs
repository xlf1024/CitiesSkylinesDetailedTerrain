            ...
            intMap = new() {
                {
                    ...
                    new() {
                        { 481 * 481, (int)((480 * f + 1) * (480 * f + 1)) },
                        { 480 * 480, (int)(480 * f * 480 * f) },
                        { 513 * 513, (int)((512 * f + 1) * (512 * f + 1)) },
                        { 4, 4 + p }, // patch rendering levels
                        { 5, 5 + p } // patch rendering levels
                    }
                },
                {
            ...
            floatMap = new() {
                ...
                },
                {
                    ...
                    new() {
                        ...
                        { 480f * 4.5f, 480f * (float)f * 4.5f },
                        { 4f * 0.5f, 4f * 0.5f / (float)f },
                        { 5.5f, 4f * UnityEngine.Mathf.Sqrt(2) / (float)f }, // sth regarding clipping paint, TODO: investigate, other paints use 4f; maybe sth like 4*sqrt(2)?
                        { 8f, 8f / (float)f }
                    }
                },
                {
                ...
                },
                {
                    ...
                    new() {
                        { 4f, 4f / (float)f },
                        { 1f / 512f, 1f / 512f / (float)f },
                        { 1f / 512f / 2f, 1f / 512f / 2f / (float)f },
                        { 480f, 480f * (float)f },
                        { 512f, 512f * (float)f }
                    }
                },
        ...
        static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> codesEnumerable) {
            ...
            for (int i = 0; i < codes.Count; i++) {
                ...
                // 2160f : map center in detailed coords
                    ...
                    code.operand = 2160f * (float)f;
                }
                //4 * sqrt(2)
                else if (code.Is(OpCodes.Ldc_R4, 5.656854f)) {
                    code.operand = 5.656854f / (float)f;
                }
                // 2 : left- and right-shift used instead of *4 and /4 to translate between detailed and undetailed coords
                    ...
                    if (codes[i + 1].opcode == OpCodes.Shl || codes[i + 1].opcode == OpCodes.Shr || codes[i + 1].opcode == OpCodes.Shr_Un) {
                        code = CodeInstructionExtensions.LoadConstant(2 + p);
                    }
                }
            ...
            for (int i = 0; i < codes.Count; i++) {
                ...
                if (code.Is(OpCodes.Ldc_I4, 480)) {
                 ...
                 // 4320: map size in detailed coords
                    ...
                    code.operand = (int)(4320 * f);
                } else if (code.Is(OpCodes.Ldc_I4, 4320 - 1)) {
                    code.operand = (int)(4320 * f - 1);
                }
                /*
                 * additional:
        ...
        static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> codesEnumerable, MethodBase method) {
            ...
            for (int i = 0; i < codes.Count; i++) {
                ...
                if (code.LoadsConstant(3)) {
                   code = CodeInstructionExtensions.LoadConstant(3 + p);
                }else if(code.LoadsConstant(4) && codes[i+1].IsLdarg(method, "level")){
                    code = CodeInstructionExtensions.LoadConstant(4 + p);
                }
                yield return code;
            }
        }
